...
Ap,const PmSparsityPattern *Bp);PmSparsityPattern *pm_copy_sparsity_pattern(
const PmSparsityPattern *input_pattern,PmAllocator *allocatorPtr);void
...
);boolean_T pm_bv_equalequals_bv(const PmBoolVector *xPtr,const PmBoolVector *
yPtr);int_T pm_create_real_vector_fields(PmRealVector *vecPtr,size_t size,
PmAllocator *allocatorPtr);PmRealVector *pm_create_real_vector(size_t
numElements,PmAllocator *allocatorPtr);PmRealVector *pm_copy_real_vector(const
PmRealVector *vecPtr,PmAllocator *allocatorPtr);void
pm_destroy_real_vector_fields(PmRealVector *vecPtr,PmAllocator *allocatorPtr);
void pm_destroy_real_vector(PmRealVector *vecPtr,PmAllocator *allocatorPtr);
int_T pm_create_int_vector_fields(PmIntVector *vecPtr,size_t size,PmAllocator *
allocatorPtr);PmIntVector *pm_create_int_vector(size_t numElements,PmAllocator
*allocatorPtr);PmIntVector *pm_copy_int_vector(const PmIntVector *vecPtr,
PmAllocator *allocatorPtr);void pm_destroy_int_vector_fields(PmIntVector *
vecPtr,PmAllocator *allocatorPtr);void pm_destroy_int_vector(PmIntVector *
vecPtr,PmAllocator *allocatorPtr);int_T pm_create_bool_vector_fields(
...
PmCharVector *vecPtr,PmAllocator *allocatorPtr);
#ifdef __cplusplus
}
#endif /* __cplusplus */
#include "mc_std.h"
...
mc_csparse_cond(const void *mcLaData);NeSystemInputSizes
...
NeDynamicSystemInput *neu_create_dynamic_system_input(
NeDynamicSystemInputSizes sizes,PmAllocator *allocatorPtr);void
...
NeDynamicSystemInput *in,PmAllocator *al);boolean_T neu_si_equalequals_si(
const NeSystemInput *destPtr,const NeSystemInput *srcPtr);boolean_T
...
mTransferTree)(const NeuDiagnosticManager *dest,const NeuDiagnosticManager *
src);const NeuDiagnosticTree *(*mGetInitialTree)(const NeuDiagnosticManager *
mgr);void(*mDestroy)(NeuDiagnosticManager *mgr);};PmfMessageId
...
const char *msg);NeuDiagnosticManager *neu_create_diagnostic_manager(
PmAllocator *alloc);typedef struct NeuDiagnosticTreePrinterTag
...
NeuDiagnosticTreePrinter *printer,const NeuDiagnosticTree *tree);void(*
mDestroy)(NeuDiagnosticTreePrinter *printer);};NeuDiagnosticTreePrinter *
...
,SSC_LOGGING_NONE,SSC_LOGGING_ALL,SSC_LOGGING_LOCAL,SSC_LOGGING_NUM}
...
mRTWModifiedTimeStamp;};typedef struct NeSolverParametersTag NeSolverParameters
...
real_T mFilteringTimeConstant;};NeSolverSystem *nes_create_crude_solver_system
(NeDynamicSystem *dynamicsystemPtr,NeModelParameters modelParameters,
NeSolverParameters solverParameters,PmAllocator *allocatorPtr);typedef enum
...
int32_T(*DiagnosisSupportMethod)(const NeSolverSystem *topSys,const
...
NeSystemInput *,const PmRealVector *d,const PmIntVector *q,PmCharVector);
...
mLinearizationSupportMethod;VslsSupportMethod mVslsSupportMethod;
EventIterationDiagMethod mEventIterationDiagMethod;}NeSolverSupportMethods;
void nes_register_solver_support(const NeSolverSupportMethods *methods);const
...
*);PmRealVector *(*mResiduals)(const NeslSimulationData *);PmRealVector *(*
mTolerances)(const NeslSimulationData *);PmSparsityPattern *(*mM_P)(const
NeslSimulationData *);PmRealVector *(*mMassMatrixPr)(const NeslSimulationData *
);PmSparsityPattern *(*mLinJacobianPattern)(const NeslSimulationData *);
PmRealVector *(*mLinJacobian)(const NeslSimulationData *);PmSparsityPattern *(
*mSolJacobianPattern)(const NeslSimulationData *);PmRealVector *(*
mSolJacobianPr)(const NeslSimulationData *);PmRealVector *(*mInputValues)(
const NeslSimulationData *);PmIntVector *(*mInputOffsets)(const
NeslSimulationData *);boolean_T *(*mFoundZcEvents)(const NeslSimulationData *)
;boolean_T *(*mIsMajorTimeStep)(const NeslSimulationData *);boolean_T *(*
mIsSolverAssertCheck)(const NeslSimulationData *);boolean_T *(*
...
NeslSimulationData *);void(*mDestroy)(NeslSimulationData *);
NeslSimulationDataData *mData;};PMF_DEPLOY_STATIC PmRealVector nesl_get_time(
const NeslSimulationData *sd){return *(sd->mTime(sd));}PMF_DEPLOY_STATIC
PmRealVector nesl_get_cont_states(const NeslSimulationData *sd){return *(sd->
mContStates(sd));}PMF_DEPLOY_STATIC PmIntVector nesl_get_mode_vector(const
NeslSimulationData *sd){return *(sd->mModeVector(sd));}PMF_DEPLOY_STATIC
PmRealVector nesl_get_disc_states(const NeslSimulationData *sd){return *(sd->
mDiscStates(sd));}PMF_DEPLOY_STATIC PmIntVector nesl_get_sample_hits(const
...
nesl_get_mass_matrix_pattern(const NeslSimulationData *sd){return *(sd->mM_P(
sd));}PMF_DEPLOY_STATIC PmRealVector nesl_get_mass_matrix_pr(const
...
nesl_get_lin_jacobian(const NeslSimulationData *sd){return *(sd->mLinJacobian(
sd));}PMF_DEPLOY_STATIC PmSparsityPattern nesl_get_sol_jacobian_pattern(const
NeslSimulationData *sd){return *(sd->mSolJacobianPattern(sd));}
PMF_DEPLOY_STATIC PmRealVector nesl_get_sol_jacobian_pr(const
NeslSimulationData *sd){return *(sd->mSolJacobianPr(sd));}PMF_DEPLOY_STATIC
PmRealVector nesl_get_input_values(const NeslSimulationData *sd){return *(sd->
mInputValues(sd));}PMF_DEPLOY_STATIC PmIntVector nesl_get_input_offsets(const
NeslSimulationData *sd){return *(sd->mInputOffsets(sd));}PMF_DEPLOY_STATIC
boolean_T nesl_found_zcs(const NeslSimulationData *sd){return *(sd->
mFoundZcEvents(sd));}PMF_DEPLOY_STATIC boolean_T nesl_is_major_time_step(const
NeslSimulationData *sd){return *(sd->mIsMajorTimeStep(sd));}PMF_DEPLOY_STATIC
boolean_T nesl_is_solver_assert_check(const NeslSimulationData *sd){return *(
...
PMF_DEPLOY_STATIC boolean_T nesl_has_sample_hits(const NeslSimulationData *sd)
{size_t i;PmIntVector sample_hits= nesl_get_sample_hits(sd);for(i= 0;i<
sample_hits.mN;i++){if(sample_hits.mX[i]){return true;}}return false;}typedef
struct McSparseMatrixTag McSparseMatrix;struct McSparseMatrixTag{
PmSparsityPattern *mPattern;PmRealVector *mPr;};McSparseMatrix *
mc_assemble_sparse_matrix(const PmSparsityPattern *patternPtr,const
PmRealVector *prPtr,PmAllocator *allocatorPtr);void
mc_disassemble_sparse_matrix(McSparseMatrix *matrix,PmAllocator *allocatorPtr)
;McSparseMatrix *mc_create_sparse_matrix(size_t nNzMax,size_t nRow,size_t nCol
,PmAllocator *allocatorPtr);McSparseMatrix *mc_copy_sparse_matrix(const
McSparseMatrix *matrixPtr,PmAllocator *allocatorPtr);McSparseMatrix *
...
const McSparseMatrix *matrix,size_t rowStart,size_t rowEndPlusOne,size_t
...
McSparseMatrix *matrix,size_t padding,PmAllocator *allocatorPtr);
McSparseMatrix *mc_create_bottom_padded_sparse_matrix(const McSparseMatrix *
matrix,size_t padding,PmAllocator *allocatorPtr);McSparseMatrix *
...
McSparseMatrix *mc_create_sparse_restriction(const McSparseMatrix *matrix,
const PmBoolVector *selectedRows,const PmBoolVector *selectedCols,PmAllocator *
...
size_t row,PmAllocator *allocatorPtr);void mc_destroy_sparse_matrix(
McSparseMatrix *matrixPtr,PmAllocator *allocatorPtr);McSparseMatrix *
mc_remove_element(const McSparseMatrix *matrix,size_t element,PmAllocator *
allocator);McSparseMatrix *mc_clean_sparse_matrix(const McSparseMatrix *in,
PmAllocator *allocator);typedef struct NeslContLtiDataTag NeslContLtiData;
struct NeslContLtiDataTag{PmRealVector *mLtiFlatU;McSparseMatrix mDx;
McSparseMatrix mDu;PmRealVector mC0;boolean_T mEmptyDx;boolean_T mEmptyDu;};
typedef struct NeCustomDataSizeTag NeCustomDataSize;typedef struct
NeCustomDataTag NeCustomData;struct NeCustomDataSizeTag{size_t mDoubleSize;
size_t mIntegerSize;};struct NeCustomDataTag{double *mDouble;int32_T *mInteger
;NeCustomDataSize mSizes;NeCustomData *mChildData;};NeCustomDataSize
ne_get_zero_custom_data_size(void);NeCustomData *ne_allocate_custom_data(
NeCustomDataSize sizes);NeCustomData *ne_allocate_empty_custom_data(void);void
ne_free_custom_data(NeCustomData *ncd);typedef struct NeParameterBundleTag{
PmIntVector mLogicalParameters;PmIntVector mIntegerParameters;PmIntVector
mIndexParameters;PmRealVector mRealParameters;}NeParameterBundle;typedef struct
NeParameterVectorTag{size_t mN;NeParameterData *mX;}NeParameterVector;typedef
struct NeParameterInfoTag{NeParameterVector mLogicals;NeParameterVector
mIntegers;NeParameterVector mIndices;NeParameterVector mReals;}NeParameterInfo
;typedef struct SlSparseMatrixTag{PmSparsityPattern *mPattern;PmRealVector *
mPr;}SlSparseMatrix;typedef struct SLMatricesTag{boolean_T
...
mMode;PmIntVector mMajor;PmIntVector mCache;}SLMatrices;typedef struct
...
NESL_SIM_RESIDUALS,NESL_SIM_TOLERANCES,NESL_NUM_SIMULATOR_METHODS}
NeslSimulatorMethodId;typedef struct NeslSimulatorTag NeslSimulator;typedef
struct NeslSimulatorUserDataTag NeslSimulatorUserData;typedef enum
NeslSimulatorStatusTag{NESL_SIM_ERROR= -1,NESL_SIM_OK}NeslSimulatorStatus;
typedef NeslSimulatorStatus(*NeslSimulatorMethod)(const NeslSimulator *,const
NeslSimulationData *,NeuDiagnosticManager *);struct NeslSimulatorTag{
NeslSimulatorUserData *mUserData;NeslSimulatorMethod mMethods[
NESL_NUM_SIMULATOR_METHODS];NeslSimulationSizes(*mSizes)(const NeslSimulator *
s);void(*mStateNames)(const NeslSimulator *s,const NeslStateNameVector *snv);
void(*mSampleTimes)(const NeslSimulator *s,PmRealVector *periods,PmRealVector *
offsets);NeslSimulatorStatus(*mInitialize)(const NeslSimulator *s,const
NeModelParameters *mp,NeuDiagnosticManager *mgr);void(*mSetupLogger)(const
NeslSimulator *s,NeLoggerBuilder *loggerBuilder);void(*mLog)(const
NeslSimulator *s,const NeslSimulationData *sd,PmRealVector *output);const
NeslContLtiData *(*mGetContLtiData)(const NeslSimulator *s);const SLMatrices *
...
NeOutputParametersTag{size_t mDaeIndex;size_t mOutputFunctionIndex;const char *
mDescriptor;}NeOutputParameters;typedef enum NeslSimulatorCategoryTag{
NESL_SIMULATOR_CATEGORY_INVALID= -1,NESL_SIMULATOR_CATEGORY_STATE,
NESL_SIMULATOR_CATEGORY_SINK,NESL_SIMULATOR_CATEGORY_OUTPUT,
...
const NeslSimulator *(*mSimulator)(const NeslSimulatorGroup *g,
NeslSimulatorCategory c,size_t i);NeslSimulatorGroup *(*mClone)(const
NeslSimulatorGroup *g);void(*mGetReference)(NeslSimulatorGroup *g);void(*
mReleaseReference)(NeslSimulatorGroup *g);const NeProfiler *mProfiler;
NeslSimulatorGroupData *mData;size_t mNumSimulators[
NESL_SIMULATOR_CATEGORY_NUM];};PmfMessageId nesl_register_simulator_group(
const char *key,const size_t numDae,NeDae *const *dae,const NeSolverParameters
*sp,const NeModelParameters *mp,const size_t numOutputs,const
NeOutputParameters *op,const boolean_T sinksNeeded);typedef struct
...
const char *key,NeslSimulatorCategory cat,size_t idx);boolean_T(*
mHasBeenCloned)(const NeslSimulatorGroupRegistry *registry,const char *key);
void(*mClear)(const NeslSimulatorGroupRegistry *registry);void(*mDestroy)(
NeslSimulatorGroupRegistry *registry);NeslSimulatorGroupRegistryData *mData;};
const NeslSimulatorGroupRegistry *nesl_get_registry(void);
#ifdef __cplusplus
}
#endif /* __cplusplus */
