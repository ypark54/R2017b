...
*mDescription;size_t mIndex;size_t mM;size_t mN;}NeParameterData;typedef struct
NeuDiagnosticTreeTag NeuDiagnosticTree;typedef struct NeuDiagnosticManagerTag
NeuDiagnosticManager;typedef enum{NEU_DIAGNOSTIC_LEVEL_INVALID= -1,
NEU_DIAGNOSTIC_LEVEL_TERSE,NEU_DIAGNOSTIC_LEVEL_VERBOSE,
NEU_NUM_DIAGNOSTIC_LEVELS}NeuDiagnosticLevel;
#ifdef __cplusplus
}
#endif /* __cplusplus */
#include "pm_inline.h"
...
extern "C" {
#endif /* __cplusplus */
typedef struct NeModelParametersTag NeModelParameters;typedef enum
...
mRTWModifiedTimeStamp;};typedef struct NeDaeTag NeDae;typedef struct
NeProfilerTag NeProfiler;typedef struct NeCustomDataSizeTag NeCustomDataSize;
typedef struct NeCustomDataTag NeCustomData;typedef struct NeRealFunctorDataTag
NeRealFunctorData;typedef struct NeRealFunctorTag NeRealFunctor;typedef struct
...
mMajor;PmIntVector mCache;}SLMatrices;typedef struct NeParameterBundleTag{
...
PmSparsityPattern mDUY_P;PmSparsityPattern mTDUY_P;PmRealVector mDUY;
PmRealVector mXP0;PmRealVector mZC;PmIntVector mASSERT;PmIntVector mPASSERT;
...
mOBS_EXP;PmRealVector mEQ_TOL;}NeDaeMethodOutput;typedef enum NeDaeMethodIdTag
...
NE_DAE_METHOD_DXY_P,NE_DAE_METHOD_DXY,NE_DAE_METHOD_DUY_P,NE_DAE_METHOD_TDUY_P
,NE_DAE_METHOD_DUY,NE_DAE_METHOD_XP0,NE_DAE_METHOD_ZC,NE_DAE_METHOD_ASSERT,
NE_DAE_METHOD_PASSERT,NE_DAE_METHOD_SO,NE_DAE_METHOD_SP,NE_DAE_METHOD_MODE,
NE_DAE_METHOD_UDOT_REQ,NE_DAE_METHOD_DELAYS,NE_DAE_METHOD_DXF_V_X,
NE_DAE_METHOD_DUF_V_X,NE_DAE_METHOD_OBS_EXP,NE_DAE_METHOD_EQ_TOL,
...
PmfMessageId(*NeDaeMethod)(const NeDae *,const NeSystemInput *,
NeDaeMethodOutput *,NeuDiagnosticManager *);typedef PmfMessageId(*NeDaeSolve)(
const NeDae *,const NeSystemInput *,NeuDiagnosticManager *);typedef
PmfMessageId(*NeDaeFunction)(const NeDae *,const NeSystemInput *);typedef
...
mNumAsserts;size_t mNumParamAsserts;size_t mNumRanges;size_t mNumAssertRanges;
size_t mNumParamAssertRanges;size_t mNumSamples;size_t mNumDelays;size_t
...
mParamAssertData;NeRange *mRanges;NeRange *mAssertRanges;NeRange *
mParamAssertRanges;NeObservableData *mObservableData;NeParameterInfo
...
void(*mSolverFailureNotify)(NeDae *dae,size_t num_consecutive_failures);void(*
mSetupLoggerFcn)(const NeDae *,NeLoggerBuilder *);int32_T(*mLogFcn)(const NeDae
*,const NeSystemInput *,PmRealVector *);void(*mGenerateFcn)(const NeDae *,
const NeCgParams *,NeCgResults *);const SLMatrices **(*mSwitchedLinearDataFcn)
(const NeDae *);NeDaeMakeOutputFcn mMakeOutput[NE_NUM_DAE_METHODS];
NeDaeFreeOutputFcn mFreeOutput[NE_NUM_DAE_METHODS];NeRealFunctorFcn
mRealFunctorFcn;void(*mSetOutputFunction)(const NeDae *,size_t
...
NeDae *dae,NeDaeMethodId id,const NeSystemInput *input,NeDaeMethodOutput *
output,NeuDiagnosticManager *mgr){return dae->mMethods[id](dae,input,output,
mgr);}PMF_DEPLOY_STATIC NeDaeMethodOutput *ne_dae_default_output(const NeDae *
dae,NeDaeMethodId id,PmAllocator *allocator){return dae->mMakeOutput[id](dae,
allocator);}PMF_DEPLOY_STATIC NeDaeMethodOutput *ne_dae_method_output(const
NeDae *dae,NeDaeMethodId id,const NeSystemInput *input,NeuDiagnosticManager *
mgr,PmAllocator *allocator){NeDaeMethodOutput *daeOutput= ne_dae_default_output
(dae,id,allocator);ne_dae_call_method(dae,id,input,daeOutput,mgr);return
daeOutput;}PMF_DEPLOY_STATIC PmfMessageId ne_dae_solve(const NeDae *dae,
NeDaeSolveId id,const NeSystemInput *input,NeuDiagnosticManager *mgr){return
dae->mSolves[id](dae,input,mgr);}PMF_DEPLOY_STATIC NeSystemInputSizes
...
}PMF_DEPLOY_STATIC void ne_dae_release_reference(NeDae *dae){;if(--(dae->
