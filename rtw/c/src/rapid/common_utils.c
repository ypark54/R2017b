...
static void deInterleaveComplex(char* realDst, char* imagDst, char* src, 
                                size_t halfElementSize, size_t numOfTimePoints) {
    size_t elIdx;
    for (elIdx=0;
         elIdx < numOfTimePoints;
         elIdx++)
    {
        /* Copy real part. */
                                              ...
                                              )
{
    ...
    if (matFileDataFormat == SINGLEVAR_MATRIX) {
        ...
        for (inportIdx = 0; inportIdx < gblNumRootInportBlks; ++inportIdx) {
            ...
            matDataRe = (char*)calloc(numOfTimePoints,elementSize);
            matDataIm = isComplex ? (char*)calloc(numOfTimePoints,elementSize) : NULL;
            if(isComplex) {
                size_t halfElementSize = elementSize/2;
                src = matData;
                realDst = matDataRe;
                imagDst = matDataIm;
                deInterleaveComplex(realDst, imagDst, src, 
                                    halfElementSize, numOfTimePoints);
            } else {
                matDataRe = matData;
            }
...
void slmrLogStatesAfterStateWrite(
    SimStruct *S,  const char* writerBlkPath)
{
    (void) S;
    (void) writerBlkPath;
}
