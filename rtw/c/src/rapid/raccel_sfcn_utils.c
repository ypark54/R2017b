...
size_t gblNumMexSFcns = 0;
size_t gblCurrentSFcnIdx = 0;
static char** gblChildIdxToSFcnName = NULL;
...
{
    const char* name =
        gblChildIdxToSFcnName[gblCurrentSFcnIdx];
...
raccelInitializeForMexSFcns()
{
    ...
    gblChildIdxToSFcnName =
        calloc(gblNumMexSFcns, sizeof(char *));
}
...
{
    *targetPtr = calloc(MAX_STR_LENGTH, sizeof(char));
    strcpy(*targetPtr, sFcnName);
}
...
{
    if (gblChildIdxToSFcnName[childIdx] == NULL)
    {
        setSFcnName(
            &gblChildIdxToSFcnName[childIdx],
            sFcnName
            );
    }
