...
#define JIT_CONTROL_C_EXCEPTION "JITContrlCException"
...
LIBMWSF_RUNTIME_API unsigned int sf_mex_listen_for_ctrl_c_with_debugger(SimStruct* S, struct SfDebugInstanceStruct* debugInstance);
...
LIBMWSF_RUNTIME_API void sf_call_atomic_subchart_user_fcn(int ssIdNumber, SimStruct* S, void* ioVarPtrs);
LIBMWSF_RUNTIME_API void sf_call_atomic_subchart_auto_fcn(int fcnNumber, SimStruct* S, void* ioVarPtrs);
LIBMWSF_RUNTIME_API mxArray* sf_call_get_hover_data_for_msg_fcn(SimStruct* chartSimStruct, int ssIdNumber);
LIBMWSF_RUNTIME_API mxArray* sf_get_data_value_from_jit_engine(SimStruct* S, int ssidNumber);

LIBMWSF_RUNTIME_API void sf_call_marshall_in_fcn(SimStruct* S, const char* fcnInName, void* var, const char*  varName, const mxArray* value);
LIBMWSF_RUNTIME_API mxArray*  sf_call_marshall_out_fcn(SimStruct* S, const char* fcnOutName, void* var,const char* varName);
...
LIBMWSF_RUNTIME_API void sfcs_call_function_with_try_catch(void* fPtr, SimStruct* S, int32_T tid, _ssFcnCallExecArgs* args);
...
LIBMWSF_RUNTIME_API void sf_set_chart_accesses_machine_info(SimStruct* S, const char* spec, mxArray* infoStruct, unsigned int chartFileNumber);
...
typedef enum {
    SF_MSG_SEND_OK = 0,
    SF_MSG_SEND_HAS_OVERFLOW  = 0x01
} SFMsgSendStatus;

void sf_check_msg_queue_for_overflow(SimStruct* S, int queueType, int queueId, int isSender,
                                     unsigned int ssIdSrcLoc, int offsetSrcLoc, int lengthSrcLoc, int overflowDiagnostic);

LIBMWSF_RUNTIME_API void sf_pre_call_check_for_send(SimStruct* S, unsigned int ssIdSrcLoc, int offsetSrcLoc, int lengthSrcLoc,
                                                    int queueType, int queueId, int isSender, int overflowDiagnostic);

LIBMWSF_RUNTIME_API void sf_pre_call_check_for_forward(SimStruct* S, unsigned int ssIdSrcLoc, int offsetSrcLoc, int lengthSrcLoc,
                                                       bool isPopped, bool isForwarded,
                                                       int queueType, int queueId, int isSender, int overflowDiagnostic);

LIBMWSF_RUNTIME_API void sf_pre_call_check_for_discard(SimStruct* S, unsigned int ssIdSrcLoc, int offsetSrcLoc, int lengthSrcLoc,
                                                       bool isPopped, bool isForwarded);

LIBMWSF_RUNTIME_API void sf_throw_overflow_information_for_local_queue(SimStruct* S, const char* queueName, int queueIdx);

LIBMWSF_RUNTIME_API int sf_get_queue_overflow_diagnostic_option(SimStruct *S, int queueType, int queueId);

LIBMWSF_RUNTIME_API void sf_register_codegen_names_for_scoped_functions_defined_by_chart(SimStruct *S);

LIBMWSF_RUNTIME_API void sf_validity_check_for_message_data_access(SimStruct* S, unsigned int ssIdSrcLoc, int offsetSrcLoc, int lengthSrcLoc,
                                                                   bool isPopped, bool isForwarded, bool isWrite);

LIBMWSF_RUNTIME_API void* getRuntimeInstanceInfoAsVoid(SimStruct* S);

LIBMWSF_RUNTIME_API void notify_msg_viewer_of_state_change(SimStruct* S, bool isEntry, unsigned int ssId);
...
LIBMWSF_RUNTIME_API void sf_error_out_about_continuous_sample_time_with_persistent_vars(SimStruct* S);
LIBMWSF_RUNTIME_API void sf_error_check_illegal_use_of_simulink_fcn(SimStruct* S, const char* subsysName, unsigned int simFcnSSID, unsigned int userSSID);
LIBMWSF_RUNTIME_API void sf_error_check_illegal_use_of_simulink_fcns(SimStruct* S, mxArray* simFcnInfo);
...
LIBMWSF_RUNTIME_API void ssSetBlockIsPurelyCombinatorial_wrapper(SimStruct* S, bool isCombinatorial);
