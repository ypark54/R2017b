...
#           else
                {
                    ssSetErrorStatus(S, _SimstructErrmsg("SimStructUnexpectedMdlOutputsInExportedFcnCall"));
                }
#           endif
            break;
...
#         if S_FUNCTION_LEVEL == 2 && defined(MDL_SIM_STATE)
	  case SS_CALL_MDL_GET_SIM_STATE:
            if (ssGetSimStateCompliance(S) != USE_CUSTOM_SIM_STATE) {
                (void)sprintf(errmsg, _SimstructErrmsgWithArg("Simulink:SFunctions:SimStructUnexpectedFunctionCall", _StrArg(%s)), "mdlGetSimState");
                _ssCopyErrorStatusToBuffer(S, errmsg, strlen(errmsg)+1);
            } else {
                mxArray** retVal;
	  ...
	  case SS_CALL_MDL_SET_SIM_STATE:
            if (ssGetSimStateCompliance(S) != USE_CUSTOM_SIM_STATE) {
                (void)sprintf(errmsg, _SimstructErrmsgWithArg("Simulink:SFunctions:SimStructUnexpectedFunctionCall", _StrArg(%s)), "mdlSetSimState");
                _ssCopyErrorStatusToBuffer(S, errmsg, strlen(errmsg)+1);
            } else {
    ...
    const char* fcn)
{
    if (*idx < idxMin || *idx >= idxMaxPlus1) {
        if (ssGetErrorStatus(S)==NULL) {  /* don't clobber an existing error */
            /* \todo: get this error from the message catalog */
            static char msg[4096];
            (void)sprintf(
                msg,
                "Error in file '%s' on line %d. "
                "The specified index (%d) in call to %s is out of bounds. "
