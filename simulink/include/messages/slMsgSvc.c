...
/* Send a message to the specified queue */
uint8_T slMsgSvcSendMsgToQueue(void* msgMgr, void* msgptr, slMsgQueueId queueId)
{
...
/* Pop the message at the top of the specified queue */
void* slMsgSvcPopMsgFromQueue(void* msgMgr, slMsgQueueId queueId, uint8_T* isOverflowed)
{
    slMsgManager* msgMgrT = (slMsgManager*)msgMgr;

    if (msgMgr == NULL || queueId == SLMSG_UNSPECIFIED) {
        if (isOverflowed != NULL) {
            *isOverflowed = (uint8_T)false;
        }
        return NULL;
    }
...
uint8_T slMsgSvcQueueIsOverflow(void* msgMgr, slMsgQueueId queueId)
{
    slMsgManager* msgMgrT = (slMsgManager*)msgMgr;
    if (queueId == SLMSG_UNSPECIFIED) {
        return (uint8_T)false;
